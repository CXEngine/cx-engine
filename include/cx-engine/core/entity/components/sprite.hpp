...
public:
    ...
    virtual void onPositionUpdate() override { sprite.setPosition(parent.getPosition()); };

    virtual void setTexture(const sf::Texture& texture, bool resetRect = false) { sprite.setTexture(texture, resetRect); }
    virtual void setTextureRect(const sf::IntRect& rect) { sprite.setTextureRect(rect); }
    virtual void setColor(const sf::Color& color) { sprite.setColor(color); }
